...
pixnetApp.controller('NewArticleCtrl', function($scope, $http, $location) {
    $scope.site_categories = [];
    $scope.personal_categories = [];
    $scope.personal_child_categories = [];
    $scope.status_list = [{ "id": "0", "name": "刪除" }, { "id": "1", "name": "草稿" }, { "id": 2, "name": "公開" }, { "id": "3", "name": "密碼" }, { "id": "4", "name": "隱藏" }, { "id": "5", "name": "好友" }, { "id": "7", "name": "共同作者" }];
    ...
    $scope.comment_hidden_list = [{ "id": "0", "name": "公開" }, { "id": "1", "name": "強制隱藏" }];
    ...
    $scope.public_at;
    $scope.category_id = 0;
    $scope.site_category_id = 0;
    $scope.comment_perm = 1;
    $scope.comment_hidden = 0;
    $scope.tags = ""; // using ',' to split
    $scope.password;
    $scope.password_hint;
    $scope.friend_group_ids;
    $scope.notify_twitter;
    $scope.notify_facebook;
    $scope.notify_plurk;
    $scope.isOpen = false;
    $scope.category;
    $scope.product;
    ...
    $scope.$watch('category', function(newValue, oldValue) {
        if (newValue != oldValue) {
            $scope.products = $scope.group_buying_categories[$scope.category].products;
        }
    });

    $scope.submit = function() {
        $scope.body = $("#body").Editor("getText");
        $scope.description = $("#group_buying_description").Editor("getText");
        var data = {
            title: $scope.title,
            body: $scope.body + '<b><a href="127.0.0.1:3000/button?isOpen=' +
                $scope.isOpen + '&price=' + $scope.price + '&article_name=' +
                $scope.title + '&category=' + $scope.category + '&prodcut_name=' +
                $scope.product + '&description=' + $scope.description + '&image=' +
                $scope.image + '&user=' + $scope.user + '&product_id=' + $scope.productId +
                '">Go Buy It</a></b><br>' +
                '<b><a href="http://140.112.26.241:81/index.html"></a></b>',
            format: $scope.format,
            status: $scope.status,
            public_at: moment($scope.public_at, 'YYYY-MM-DD HH:mm:ss').format('X'),
            category_id: $scope.category_id,
            site_category_id: $scope.site_category_id,
            comment_perm: $scope.comment_perm,
            comment_hidden: $scope.comment_hidden,
            tags: $scope.tags, // using ',' to split
            password: $scope.password,
            password_hint: $scope.password_hint,
            friend_group_ids: $scope.friend_group_ids,
            notify_twitter: $scope.notify_twitter,
            notify_facebook: $scope.notify_facebook,
            nitify_plurk: $scope.notify_plurk,

            isOpen: $scope.isOpen,
            price: $scope.price,
            published: $scope.moment($scope.public_at, 'YYYY/MM/DD'),
            article_title: $scope.title,
        ...
        $http({ method: "POST", url: "/submitpost", data: data }).success(function(res) {
            console.log(res);
